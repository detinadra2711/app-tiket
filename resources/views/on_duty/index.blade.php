...
@section('head')
LIST OF ON DUTY RESERVATION
@endsection

@section('content')

@auth
    <a href="/tiket_onduty/create" class="btn bg-indigo btn-labeled btn-labeled-left">Tambah Reservasi</a> <br>
    {{-- <a href="/tiket_onduty/create" class="btn btn-indigo btn-sm mb-4">Tambah Reservasi</a> --}}
<br><h6>
  MASKAPAI : <br>
  1. Batik + GA <br>
  2. Batik <br>
  ...
  <tbody>
    ...
    @forelse ($on_duty as $key => $value)
        <tr>
            ...
            @auth
               <td>
                <form action="/tiket_onduty/{{ $value->id }}" method="POST">
                    {{-- <a href= "/helpdesk/{{ $value->id }}" class="btn btn-success btn-sm">Simpan</a> --}}
                    <a href= "/tiket_onduty/{{ $value->id }}" class="btn btn-info btn-sm">Detail</a>
